...
export default function Counter() {
  ...
  const [color, dispatchColor] = useReducer(colorReducer, pinkRGB)

  useEffect(() => {
    if (count === 0) {
      dispatchColor({ type: ACTIONS.SET_COLOR, payload: { color: pinkRGB } })
      // setCurrentColor(pinkRGB)
    }

    if (count > 0) {
      dispatchColor({ type: ACTIONS.SET_COLOR, payload: { color: `rgb(52, 211, 153)` } })
      // setCurrentColor(`rgb(52, 211, 153)`)
    }

    if (count < 0) {
      dispatchColor({ type: ACTIONS.SET_COLOR, payload: { color: `rgb(239, 68, 68)` } })
